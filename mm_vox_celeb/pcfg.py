...
def generate(pred, n = 10):
    ...
    is_list = [GET_NAME[a] for a in attr if ATTR_VERB[a] == 'is' and a != 'Male']
    ...
    for i in range(n):
        ...
        for attr_t in attr_tuple:
            male = (pred[GENDER_IDX], 0.5) if first else (pred[GENDER_IDX], 0.85)
            first = False
...
def generate_random_sentences(n_attr=8, n_sent=16):
    ...
    for _ in range(n_sent):
        ...
        pred = mutual_exclusive(pred, ['Black_Hair', 'Blond_Hair', 'Brown_Hair', 'Gray_Hair'])
