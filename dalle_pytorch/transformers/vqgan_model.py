...
class VQGanTransformer(nn.Module):
    def __init__(self, which_model='vqgan_drin', seq_len=0, causal=True, mask_type='causal', mask_kwargs={}):
        super().__init__()
        ...
        transformer_config = config['model']['params']['transformer_config']['params']
        ...
        state_dict = {}
        for k in state.keys():
            if k.startswith('transformer.') and 'attn.mask' not in k and 'pos_emb' not in k:
...
class minGPTTransformer(nn.Module):
    def __init__(self, which_model='mingpt', seq_len=0, causal=True, mask_type='causal', mask_kwargs={}, n_head=8, n_layers=24, n_emb=512):
        super().__init__()

        self.causal = causal
        # model = instantiate_from_config(config=transformer_config)
        model = GPT(vocab_size=1, block_size=seq_len, n_layer=n_layers, n_head=n_head, n_embd=n_emb,
            causal=causal, input_is_embd=True, mask_type=mask_type, mask_kwargs=mask_kwargs)
        self.model = model
