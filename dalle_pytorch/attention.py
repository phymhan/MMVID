...
class SparseConvCausalAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        padding = seq_len - n + 1
        mask = default(mask,
                       lambda: torch.ones(b, text_len, device=device).bool())
        ...
        text_causal_mask = torch.ones(i, j,
                                      device=device).triu_(j - i + 1).bool()
...
class SparseAxialCausalAttention(nn.Module):
    def __init__(self,
        ...
        assert axis in {
            0, 1
        }, 'axis must be either 0 (along height) or 1 (along width)'
        self.axis = axis
    ...
    def forward(self, x, mask=None):
        ...
        bh, x, i, j = dots.shape
        causal_mask = torch.ones(i, img_size,
                                 device=device).triu_(img_size - i + 1).bool()
